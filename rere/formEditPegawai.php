...
<form method="POST" action="controllers/pegawaiController.php">
    <div class="form-group row">
        ...
        <div class="col-8">
            <div class="input-group">
                ...
                <input id="nip" name="nip" value="<?= $edit_data['nip'] ?>" type="text" class="form-control" required="required">
            </div>
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-8">
            <input id="nama" name="nama" value="<?= $edit_data['nama'] ?>" placeholder="Nama Anda!" type="text" class="form-control" required="required">
        </div>
    </div>
    <div class="form-group row">
        <label for="email" class="col-4 col-form-label">Email</label>
        <div class="col-8">
            <input id="email" name="email" value="<?= $edit_data['email'] ?>" placeholder="Email Anda!" type="text" class="form-control" required="required">
        </div>
    </div>
    <div class="form-group row ">
    ...
    <div class="form-group row">
        ...
        <div class="col-8">
            <select id="divisi" name="divisi" class="custom-select" required="required">
                ...
                foreach ($rs as $sahid) {
                    //edit divisi
                    $sel_divisi = ($edit_data['iddivisi'] == $sahid['id']) ? "selected" : "";
                ?>
                    <option value="<?= $sahid['id'] ?>" <?= $sel_divisi ?>> <?= $sahid['nama'] ?> </option>
                <?php } ?>
            </select>
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-8">
            <input id="foto" name="foto" value="<?= $edit_data['foto'] ?>" placeholder="Foto Anda!" type="text" class="form-control">
        </div>
    </div>
    <div class="form-group row">
        <div class="offset-4 col-8">
            <button name="proses" type="submit" value="edit" class="btn btn-primary"><i class="far fa-edit"></i> Edit</button>
            <input type="hidden" name="edi" value="<?= $id ?>">
        </div>
    </div>
</form>
